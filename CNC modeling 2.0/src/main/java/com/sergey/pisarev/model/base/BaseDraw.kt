...
abstract class BaseDraw : ProgramText {
    ...
    private val lineWidth = 1.5
    private val lineWidthDashes = 1.0

    protected var isNumberLine = false
    private val colorLine = Color.GREEN
    private val colorLineDashes = Color.BLACK
    ...
    protected fun drawArc(gc: GraphicsContext, isRapidFeed: Int, pointSystemCoordinate: Point, pointStart: Point, pointEnd: Point, radius: Double, zoom: Double, clockwise: Int) {
        var radius = radius
        if (isRapidFeed == 0) {
        ...
        pEnd.z = pEnd.z * zoom
        radius *= zoom
        val chord = Math.sqrt(Math.pow(pStart.x - pEnd.x, 2.0) + Math.pow(pStart.z - pEnd.z, 2.0))
        val sweetAngle = 2 * Math.asin(chord / (2 * radius)) * (180 / Math.PI)
        val h = Math.sqrt(radius * radius - chord / 2 * (chord / 2))
        if (clockwise == 2) {
    ...
    private fun calculateStartAngle(gc: GraphicsContext, pointSystemCoordinate: Point, radius: Double, pStart: Point, sweetAngle: Double, x01: Double, z01: Double) {
        var cathet: Double
        var startAngle = 0.0
        if (pStart.x > x01 && pStart.z >= z01) {
            cathet = pStart.x - x01
            startAngle = if (pStart.z == z01) 0.0 else 360 - Math.acos(cathet / radius) * (180 / Math.PI)
        }
        if (pStart.x >= x01 && pStart.z < z01) {
            cathet = pStart.x - x01
            startAngle = if (pStart.x == x01) 90.0 else Math.acos(cathet / radius) * (180 / Math.PI)
        }
        if (pStart.x < x01 && pStart.z <= z01) {
            cathet = x01 - pStart.x
            startAngle = if (pStart.z == z01) 180.0 else 180 - Math.acos(cathet / radius) * (180 / Math.PI)
        }
        if (pStart.x <= x01 && pStart.z > z01) {
            cathet = x01 - pStart.x
            startAngle = if (pStart.x == x01) 270.0 else 180 + Math.acos(cathet / radius) * (180 / Math.PI)
        }
        gc.strokeArc(pointSystemCoordinate.x + x01 - radius, pointSystemCoordinate.z - z01 - radius, radius * 2, radius * 2, 360 - startAngle - sweetAngle, sweetAngle, ArcType.OPEN)
    }

    protected fun drawLine(gc: GraphicsContext, isRapidFeed: Int, pointSystemCoordinate: Point, pointStart: Point, pointEnd: Point, zoom: Double) {
        if (isRapidFeed == 0) {
            gc.stroke = colorLineDashes
            gc.setLineDashes(3.0, 5.0)
            gc.lineWidth = lineWidthDashes
        ...
        pEnd.x = pEnd.x * zoom
        pEnd.z = pEnd.z * zoom
        if (pStart.z > 0) pStart.z = pointSystemCoordinate.z - pStart.z else pStart.z = pointSystemCoordinate.z + Math.abs(pStart.z)
        if (pEnd.z > 0) pEnd.z = pointSystemCoordinate.z - pEnd.z else pEnd.z = pointSystemCoordinate.z + Math.abs(pEnd.z)
        gc.strokeLine(pointSystemCoordinate.x + pStart.x, pStart.z, pointSystemCoordinate.x + pEnd.x, pEnd.z)
    }

    protected fun drawPoint(gc: GraphicsContext, pointSystemCoordinate: Point, pointEnd: Point, zoom: Double, color: Color?) {
        var radiusPoint = 4.0
        val pEnd = Point(pointEnd.x, pointEnd.z)
        pEnd.x = pEnd.x * zoom
        pEnd.z = pEnd.z * zoom
        if (pEnd.z > 0) pEnd.z = pointSystemCoordinate.z - pEnd.z else pEnd.z = pointSystemCoordinate.z + Math.abs(pEnd.z)
        gc.fill = color
        gc.fillOval(pointSystemCoordinate.x + pEnd.x - radiusPoint, pEnd.z - radiusPoint, radiusPoint * 2, radiusPoint * 2)
        gc.stroke = color
        gc.setLineDashes()
        gc.lineWidth = lineWidth
        radiusPoint += 2.5
        gc.strokeOval(pointSystemCoordinate.x + pEnd.x - radiusPoint, pEnd.z - radiusPoint, radiusPoint * 2, radiusPoint * 2)
    }

    protected fun drawPoint(gc: GraphicsContext, pointSystemCoordinate: Point, frameList: List<Frame>, zoom: Double, color: Color?, index: Int, tool: String) {
        var radiusPoint = 6.0
        for (toolStr in toolsMap.keys) {
            if (tool == toolStr) radiusPoint = toolsMap[toolStr]!!
        }
        pEnd = toolRadiusCompensationPoint(frameList, radiusPoint, index)
        if (frameList[index - 1].isAxisContains && isToolRadiusCompensation != 0 && radiusPoint == 10.0 || radiusPoint == 16.0 || radiusPoint == 6.0 || radiusPoint == 12.5) {
            radiusPoint *= zoom
            pEnd.x = pEnd.x * zoom
            pEnd.z = pEnd.z * zoom
            if (pEnd.z > 0) pEnd.z = pointSystemCoordinate.z - pEnd.z else pEnd.z = pointSystemCoordinate.z + Math.abs(pEnd.z)
        ...
        if (frameList[index - 1].isAxisContains && isToolRadiusCompensation != 0 && radiusPoint == 3.0) {
            radiusPoint *= zoom
            pEnd.x = pEnd.x * zoom
            pEnd.z = pEnd.z * zoom
            if (pEnd.z > 0) pEnd.z = pointSystemCoordinate.z - pEnd.z else pEnd.z = pointSystemCoordinate.z + Math.abs(pEnd.z)
            gc.fill = color
            gc.fillOval(pointSystemCoordinate.x + pEnd.x - radiusPoint, pEnd.z - radiusPoint, radiusPoint * 2, radiusPoint * 2)
            gc.fillOval(pointSystemCoordinate.x + pEnd.x - radiusPoint + 18.352591 * zoom, pEnd.z - radiusPoint - 4.917562 * zoom, radiusPoint * 2, radiusPoint * 2)
            gc.fillOval(pointSystemCoordinate.x + pEnd.x - radiusPoint + 13.435029 * zoom, pEnd.z - radiusPoint - 23.270153 * zoom, radiusPoint * 2, radiusPoint * 2)
            gc.fillOval(pointSystemCoordinate.x + pEnd.x - radiusPoint - 4.917562 * zoom, pEnd.z - radiusPoint - 18.352591 * zoom, radiusPoint * 2, radiusPoint * 2)
            gc.beginPath()
            ...
            radiusPoint = 3.5
            radiusPoint *= zoom
            gc.strokeOval(pointSystemCoordinate.x + pEnd.x - radiusPoint + 6.717514 * zoom, pEnd.z - radiusPoint - 11.635076 * zoom, radiusPoint * 2, radiusPoint * 2)
        }
        if (frameList[index - 1].isAxisContains && isToolRadiusCompensation != 0 && radiusPoint == 1.6) {
            radiusPoint *= zoom
            pEnd.x = pEnd.x * zoom
            pEnd.z = pEnd.z * zoom
            if (pEnd.z > 0) pEnd.z = pointSystemCoordinate.z - pEnd.z else pEnd.z = pointSystemCoordinate.z + Math.abs(pEnd.z)
            gc.fill = color
            gc.fillOval(pointSystemCoordinate.x + pEnd.x - radiusPoint, pEnd.z - radiusPoint, radiusPoint * 2, radiusPoint * 2)
            gc.fillOval(pointSystemCoordinate.x + pEnd.x - radiusPoint - 1.132803 * zoom, pEnd.z - radiusPoint - 12.948001 * zoom, radiusPoint * 2, radiusPoint * 2)
            gc.fillOval(pointSystemCoordinate.x + pEnd.x - radiusPoint - 13.834918 * zoom, pEnd.z - radiusPoint - 14.059292 * zoom, radiusPoint * 2, radiusPoint * 2)
            gc.fillOval(pointSystemCoordinate.x + pEnd.x - radiusPoint - 12.702115 * zoom, pEnd.z - radiusPoint - 1.111291 * zoom, radiusPoint * 2, radiusPoint * 2)
            gc.beginPath()
            ...
            radiusPoint = 2.5
            radiusPoint *= zoom
            gc.strokeOval(pointSystemCoordinate.x + pEnd.x - radiusPoint - 6.917459 * zoom, pEnd.z - radiusPoint - 7.029646 * zoom, radiusPoint * 2, radiusPoint * 2)
        }
        if (frameList[index - 1].isAxisContains && isToolRadiusCompensation != 0 && radiusPoint == 1.2) {
            radiusPoint *= zoom
            pEnd.x = pEnd.x * zoom
            pEnd.z = pEnd.z * zoom
            if (pEnd.z > 0) pEnd.z = pointSystemCoordinate.z - pEnd.z else pEnd.z = pointSystemCoordinate.z + Math.abs(pEnd.z)
            gc.fill = color
            gc.fillOval(pointSystemCoordinate.x + pEnd.x - radiusPoint, pEnd.z - radiusPoint, radiusPoint * 2, radiusPoint * 2)
            gc.fillOval(pointSystemCoordinate.x + pEnd.x - radiusPoint - 7.892323 * zoom, pEnd.z - radiusPoint - 21.099913 * zoom, radiusPoint * 2, radiusPoint * 2)
            radiusPoint = 2.0
            radiusPoint *= zoom
            gc.fillOval(pointSystemCoordinate.x + pEnd.x - radiusPoint - 6.488786 * zoom, pEnd.z - radiusPoint - 9.604683 * zoom, radiusPoint * 2, radiusPoint * 2)
            gc.fillOval(pointSystemCoordinate.x + pEnd.x - radiusPoint - 1.403537 * zoom, pEnd.z - radiusPoint - 11.49523 * zoom, radiusPoint * 2, radiusPoint * 2)
            gc.beginPath()
            ...
            radiusPoint = 2.5
            radiusPoint *= zoom
            gc.strokeOval(pointSystemCoordinate.x + pEnd.x - radiusPoint - 3.946162 * zoom, pEnd.z - radiusPoint - 10.549956 * zoom, radiusPoint * 2, radiusPoint * 2)
        }
    }

    protected fun drawRND(gc: GraphicsContext, isRapidFeed: Int, pointSystemCoordinate: Point, pointStart: Point, pointEnd: Point, pointF: Point, radiusRND: Double, zoom: Double) {
        val pointStartCR = Point()
        val pointEndCR = Point()
        val cathet: Double
        var clockwiseRND = 3
        val angle = Point2D(pointEnd.x - pointStart.x, pointEnd.z - pointStart.z).angle(pointEnd.x - pointF.x, pointEnd.z - pointF.z)
    ...
    protected fun correctionForOffn(frameList: List<Frame>) {
        for (i in frameList.indices) {
            ...
            if (frameList[i].isCR && frameList[i].isAxisContains && frameList[i].offn > 0) {   //drawArc                            //draw Arc
    ...
    protected fun toolRadiusCompensationArcOffn(frameList: List<Frame>, numberLIne: Int, isToolRadiusCompensation: Int, clockwise: Int) {
        ...
        val chord = Math.sqrt(Math.pow(pStart.x - pEnd.x, 2.0) + Math.pow(pStart.z - pEnd.z, 2.0))
        var h = Math.sqrt(radius * radius - chord / 2 * (chord / 2))
    ...
    private fun calculateToolRadiusCompensationClockwise(frameList: List<Frame>, numberLIne: Int, radius: Double, offn: Double, x01: Double, z01: Double) {
    ...
    private fun calculateToolRadiusCompensationСounterclockwise(frameList: List<Frame>, numberLIne: Int, radius: Double, offn: Double, x01: Double, z01: Double) {
        val tempStartX = (x01 - pStart.x) * ((radius - offn) / radius)
        val tempStartZ = (z01 - pStart.z) * ((radius - offn) / radius)
        val tempEndX = (x01 - pEnd.x) * ((radius - offn) / radius)
        val tempEndZ = (z01 - pEnd.z) * ((radius - offn) / radius)
    ...
    protected fun toolRadiusCompensationLineOffn(frameList: List<Frame>, numberLIne: Int, isToolRadiusCompensation: Int) {
        val offn = frameList[numberLIne].offn
        pEnd.x = frameList[numberLIne].x
        pEnd.z = frameList[numberLIne].z
        if (isToolRadiusCompensation == 1) {
            ...
            if (pStart.x < pEnd.x && pStart.z > pEnd.z) {
                ...
                angle = 180 - 90 - angle
                val cathet1 = offn * Math.sin(Math.toRadians(angle))
                val cathet2 = Math.sqrt(offn * offn - cathet1 * cathet1)
                if (!containsG41G42(frameList[numberLIne].gCode)) {
    ...
    private fun toolRadiusCompensationPoint(frameList: List<Frame>, radiusPoint: Double, index: Int): Point {
        ...
        val pStart: Point
        val pEnd: Point
        if (containsG41G42(frameList[index - 1].gCode)) {
            pStart = Point(frameList[index - 1].x, frameList[index - 1].z)
            pEnd = Point(frameList[index].x, frameList[index].z)
            checkGCode(frameList[index].gCode)
        } else {
            pStart = Point(frameList[index - 2].x, frameList[index - 2].z)
            pEnd = Point(frameList[index - 1].x, frameList[index - 1].z)
            checkGCode(frameList[index - 1].gCode)
        }
        if (frameList[index].isCR && frameList[index].isAxisContains && containsG41G42(frameList[index - 1].gCode)) {
            val radius = frameList[index].cr
            calculateChord(radiusPoint, pStart, pEnd, radius, pStart.x, pStart.z)
        } else if (frameList[index - 1].isCR && frameList[index - 1].isAxisContains && !containsG41G42(frameList[index - 1].gCode)) {
            val radius = frameList[index - 1].cr
        ...
        if (!frameList[index].isCR && frameList[index].isAxisContains && containsG41G42(frameList[index - 1].gCode)) {
            if (isToolRadiusCompensation == 1) {
                ...
                if (pStart.x < pEnd.x && pStart.z > pEnd.z) {
                    var angle = Point2D(pEnd.x - pStart.x, pStart.z - pEnd.z).angle(pStart.x, 0.0)
                    angle = 180 - 90 - angle
                    val cathet1 = radiusPoint * Math.sin(Math.toRadians(angle))
                    val cathet2 = Math.sqrt(radiusPoint * radiusPoint - cathet1 * cathet1)
        ...
        } else if (!frameList[index - 1].isCR && frameList[index - 1].isAxisContains && !containsG41G42(frameList[index - 1].gCode)) {    //draw line
            if (isToolRadiusCompensation == 1) {
                if (pStart.z == pEnd.z && pStart.x > pEnd.x) {
            ...
            if (isToolRadiusCompensation == 2) {
                ...
                if (pStart.z == pEnd.z && pStart.x < pEnd.x) {
                    if (frameList[index - 1].x == frameList[index].x && !frameList[index].gCode.contains("G0")) {
                        pEnd.x = pEnd.x - radiusPoint
    ...
    private fun calculateChord(radiusPoint: Double, pStart: Point, pEnd: Point, radius: Double, x: Double, z: Double) {
        val chord = Math.sqrt(Math.pow(pStart.x - pEnd.x, 2.0) + Math.pow(pStart.z - pEnd.z, 2.0))
        val h = Math.sqrt(radius * radius - chord / 2 * (chord / 2)).toFloat()
        if (clockwise == 2) {
            val x01 = pStart.x + (pEnd.x - pStart.x) / 2 + h * (pEnd.z - pStart.z) / chord
            val z01 = pStart.z + (pEnd.z - pStart.z) / 2 - h * (pEnd.x - pStart.x) / chord
            if (isToolRadiusCompensation == 1) {
